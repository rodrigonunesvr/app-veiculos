...
export default function Admin() {
    ...
    const exportCSV = () => {
        ...
        const headers = ['Placa', 'Condutor', 'Entrada', 'SaÃ­da']
        const csvContent = [
            headers.join(','),
            ...events.map(e => [
                e.plate,
                ...
                e.exit_at ? new Date(e.exit_at).toLocaleString() : 'Em pÃ¡tio'
            ].join(','))
        ].join('\\n')
    ...
    return (
        <div className="space-y-4">
            < div className ="flex justify-between items-center">
                < h2 className ="text-xl font-bold">RelatÃ³rio</h2>
                    ...
                    <th className="p-3 text-left">Placa</th>
                        < th className ="p-3 text-left">Condutor</th>
                            < th className ="p-3 text-left">Entrada</th>
                                < th className ="p-3 text-left">SaÃ­da</th>
            </tr >
          </thead >
          <tbody>
            {events.map(e => (
        ...
        < td className ="p-3">{new Date(e.entry_at).toLocaleString('pt-BR')}</td>
            < td className ="p-3">
    {
        e.exit_at ? new Date(e.exit_at).toLocaleString('pt-BR') : <span className="text-green-600 font-bold">No PÃ¡tio</span>}
                </td >
              </tr >
            ))
    }
