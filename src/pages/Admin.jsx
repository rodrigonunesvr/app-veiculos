...
export default function Admin() {
  ...
  const exportPDF = () => {
    ...
    const tableColumn = ["Ocorrência", "Sistema", "Ação", "Tipo", "ID", "Condutor", "Destino", "Funcionário"]
    const tableRows = []

    movements.forEach(m => {
      const staff = m.staff_full_name ? `${m.staff_full_name} (${m.staff_rg || 'S/RG'})` : 'Sistema'
      const isRetroactive = Math.abs(new Date(m.created_at) - new Date(m.event_at)) > 5 * 60 * 1000
      const retroStatus = isRetroactive ? "(R)" : ""
      
      const row = [
        new Date(m.event_at).toLocaleString('pt-BR'),
        new Date(m.created_at).toLocaleString('pt-BR'),
        (m.direction === 'ENTRY' ? 'ENTRADA' : 'SAÍDA') + (isRetroactive ? " (R)" : ""),
        m.subject_type,
        m.subject_code,
    ...
    <div className="bg-white rounded-lg shadow overflow-x-auto">
      < table className ="min-w-full text-sm">
        < thead className ="bg-gray-50 border-b">
          ...
          <th className="p-3 text-left">Sistema (Audit)</th>
...
}
{
  ...
  < td className ="p-3 text-xs">
    <div>{new Date(m.event_at).toLocaleString('pt-BR')}</div>
  </td>
  <td className="p-3 text-xs">
    <div className="flex flex-col gap-1">
      {new Date(m.created_at).toLocaleString('pt-BR')}
      {Math.abs(new Date(m.created_at) - new Date(m.event_at)) > 5 * 60 * 1000 && (
        <span className="bg-yellow-100 text-yellow-800 text-[10px] font-bold px-1 rounded border border-yellow-200 w-fit">
          RETROATIVO
        </span>
      )}
    </div>
